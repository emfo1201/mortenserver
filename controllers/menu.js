...
export const addCategory = (req, res) => {
    ...
    Menu.findOne({ mainMenu: req.body.newCategory })
        .then(async menu => {
            if (menu) {
...
export const addSubCategory = async (req, res)  => {
    try{
        console.log("in addSubCategory ", req.body.category, " ", req.body.newSubCategory)
        Menu.findOneAndUpdate({mainMenu: req.body.category}, {$addToSet: { subMenu: req.body.newSubCategory }},
            {new: true, useFindAndModify: false}, function (err, menu) {
                if (err) {
                    console.log(err);
...
export const deleteSubCategory = async (req, res) => {
    req.body.subCategory.forEach(e => {
        try{
            Menu.findOneAndUpdate({mainMenu: e.main}, {$pull: { subMenu: e.sub }},
                {new: true, useFindAndModify: false}, function (err, menu) {
                    if (err) {
                        console.log(err)
                        return res.sendStatus(500)
